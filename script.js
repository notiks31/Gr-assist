...
let routePolyline = null; // 경로 선 관리 (새로 추가)
...
async function searchRoute() {
    ...
    try {
        ...
        if (routes && routes.length > 0) {
            ...
            // 📢 최선의 경로 (첫 번째 경로)의 Polyline을 지도에 그립니다.
...
function drawPolyline(coords) {
    ...
    routePolyline = new kakao.maps.Polyline({
        path: linePath, 
        strokeWeight: 7, // 경로선을 더 두껍게
        strokeColor: '#0070c0', // 파란색 경로선
        strokeOpacity: 0.8, 
        strokeStyle: 'solid' 
    });
...
function displayDetailedRoute() {
    ...
    // 📢 디자인 시뮬레이션을 위한 하드코딩된 HTML 구조 (이미지 2, 3 기반)
    let html = `
